...
HRESULT PrjStartDirectoryEnumerationCb(const PRJ_CALLBACK_DATA* callbackData, const GUID* enumerationId) {
	...
	if (fileName == L"test") return S_OK;
...
int main() {
	...
	std::string str;
	cout << "Virtual FS started. Now try to delete directory c:\\temp\\virt1\\test or "
		<< " 'cd /mnt/c/temp/virt1' and 'mkdir dir123' from WSL1" << endl;
